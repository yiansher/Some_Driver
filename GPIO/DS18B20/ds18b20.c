...
uint8_t DS18B20_Reset()
{
    ...
    DQ_L();
    delay_us(520); // 480~960us, reset pulse
    DQ_H();
    delay_us(15); // 15~60us, enter rx mode
    ...
    retry = 0;
    while (DQ_IS_L())
    { // presence pulse(a low signal for 60~240us)
        delay_us(10);
        if (++retry > 24)
            return 0;
    }
...
int16_t DS18B20_ReadTempReg(void)
{
    ...
    DS18B20_WriteByte(CMD_READ_ROM_UNUSE_ID);
    DS18B20_WriteByte(CMD_CONVERT_TEMP); // 温度转换命令
    ...
    DS18B20_WriteByte(CMD_READ_ROM_UNUSE_ID);
    DS18B20_WriteByte(CMD_READ_REGISTER); // 读温度寄存器
...
int16_t DS18B20_ReadTempRegByID(uint8_t *id)
{
    ...
    // Match Rom
    DS18B20_WriteByte(CMD_MATCH_SLAVE);
    for (i = 0; i < 8; i++)
    ...
    DS18B20_WriteByte(CMD_READ_REGISTER);
...
{ // 精度设置
    ...
    // 写暂存器指令
    DS18B20_WriteByte(CMD_WRITE_REGISTER);
...
uint8_t DS18B20_ReadTempRegCRC(int16_t *_temp)
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        DS18B20_WriteByte(CMD_READ_ROM_UNUSE_ID); /* 发命令 */
        DS18B20_WriteByte(CMD_CONVERT_TEMP);                  /* 发转换命令 */
